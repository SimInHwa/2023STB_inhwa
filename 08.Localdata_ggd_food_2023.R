...
#날짜데이터를 분석용 데이터로 변경
#1.YYYYMMDD형식으로 변경
ggd_food_2023$open_date <- gsub("-","",ggd_food_2023$open_date)
ggd_food_2023$close_date <- gsub("-","",ggd_food_2023$close_date)

#2.문자형 데이터를 정수형 데이터로 변환
ggd_food_2023$open_date <- as.integer(ggd_food_2023$open_date)
ggd_food_2023$close_date <- as.integer(ggd_food_2023$close_date)
...
#2.type변수
table(ggd_food_2023$type)
#처리결과 확인
table(ggd_food_2023$type)

#3.open_date변수
range(ggd_food_2023$open_date, na.rm = T)
table(is.na(ggd_food_2023$open_date))#결측치 없음
ggd_food_2023$open_year<-substr(ggd_food_2023$open_date,1,4)#인허가년도 변수 생성

#4.close_date변수
range(ggd_food_2023$close_date, na.rm = T)
ggd_food_2023$close_year<-substr(ggd_food_2023$close_date,1,4)#인허가년도 변수 생성

#5.address변수
ggd_food_2023$district<-substr(ggd_food_2023$address,7,9)#구 정보를 분리하여 변수 생성
table(ggd_food_2023$district)#이상치 확인
ggd_food_2023$district <- ifelse(ggd_food_2023$district%in%c("시 분","시 팔","주시 ","시 분","시 동","시 중", "시 일", "시 수", "시 단", "시 덕", "부시 ", "시 영", "시 상"),NA,ggd_food_2023$district)#이상치제거
table(ggd_food_2023$district)#이상치 확인
#최종 확인
str(ggd_food_2023)
#문자형데이터를 정수형으로 변경
ggd_food_2023$open_year <- as.integer(ggd_food_2023$open_year)
ggd_food_2023$close_year <- as.integer(ggd_food_2023$close_year)
str(ggd_food_2023)
...
district_business<-ggd_food_2023 %>%
